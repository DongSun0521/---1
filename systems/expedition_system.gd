...
func create_action_report(daily_report: Dictionary, state: Dictionary) -> Dictionary:
	return {
		...
		"daily_summary_lines": daily_report.get("daily_summary_lines", []).duplicate(true),
		"resource_changes": daily_report.get("resource_changes", {}).duplicate(true),
		"resource_change_details": daily_report.get("resource_change_details", {}).duplicate(true),
		"farm_events": daily_report.get("farm_events", []).duplicate(true),
		"food_workshop_events": daily_report.get("food_workshop_events", []).duplicate(true),
		"hospital_events": daily_report.get("hospital_events", []).duplicate(true),
		"weapon_forge_events": daily_report.get("weapon_forge_events", []).duplicate(true),
		"construction_events": daily_report.get("construction_events", []).duplicate(true),
		"village_consumption_events": daily_report.get("village_consumption_events", []).duplicate(true),
		"expedition_consumption_events": daily_report.get("expedition_consumption_events", []).duplicate(true),
		"hospital_report": daily_report.get("hospital_report", {}).duplicate(true),
		"forge_report": daily_report.get("forge_report", {}).duplicate(true),
