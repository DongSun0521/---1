...
func launch_projectile(projectile_id: StringName, source_view, target_view) -> bool:
	...
	if data == null:
		return false
	var source_position := effect_player.get_view_anchor(source_view, data.source_anchor) + data.source_offset
	var target_position := effect_player.get_view_anchor(target_view, data.target_anchor) + data.target_offset
	...
	tween.tween_property(projectile, "position", local_target, data.travel_duration).set_trans(Tween.TRANS_LINEAR)
	await tween.finished
	if handle.is_completed:
		return false
	effect_player.stop_effect(handle)
	if data.impact_effect_id != &"":
		var impact_context := ContextScript.new()
		impact_context.source_view = source_view
		impact_context.target_view = target_view
		last_impact_handle = effect_player.play_effect(data.impact_effect_id, impact_context)
